...
import { ContextMenu } from './Provider';
...
export function DropdownRoot() {
  const { options, cachedId, getOptionById, targetId } = useContext(ContextMenu);
      ...
      >
        ...
        >
          ...
          <motion.div
            ...
            transition={{
              x: isFirstInteraction ? { dureation: 0 } : undefined,
            }}
          >
...
function DropdownArrow({ isFirstInteraction }) {
  const { cachedId, getOptionById } = useContext(ContextMenu);
